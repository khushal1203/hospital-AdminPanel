...
export async function POST(request) {
  try {
    ...
    if (reportName) {
      const existingIndex = donor.documents[sectionKey].findIndex(
        (doc) => doc.reportName === reportName,
      );
      if (existingIndex !== -1) {
    ...
    const existingReportName =
      donor.documents[sectionKey][targetIndex]?.reportName;
