...
export async function POST(req) {
  try {
    ...
    if (
      ...
      user.resetPasswordLastAttempt > twentyFourHoursAgo
    ) {
        ...
        return NextResponse.json(
          {
            success: false,
            message:
              "You have reset password multiple times. Please try later after 24 hours.",
          },
          { status: 429 },
        );
      }
    ...
    const resetUrl = `${req.headers.get("origin") || process.env.NEXT_PUBLIC_APP_URL}/auth/reset-password?token=${resetToken}`;
